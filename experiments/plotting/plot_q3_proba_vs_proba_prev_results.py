...
os.makedirs('experiments/plots/q3_appendix', exist_ok=True)
...
plt.axis('off')
plt.savefig('experiments/plots/q3_appendix/legend.png', bbox_inches='tight', dpi=300)
plt.close()

# Dictionary to store percentages for averaging
percentages = {feature: {classifier: {'proba': [], 'proba_prev': []} for classifier in classifiers} 
...
for benchmark in baseline_data['growacq'].keys():
    ...
    for feature in ['simple_rel', 'rel_dim', 'rel_dim_block']:
        ...
        x = np.arange(2)  # Two positions for proba and proba_prev
        ...
        for classifier in classifiers:
            ...
            # Get proba_prev value
            if ('obj_proba' in classifier_data['growacq'][benchmark] and 
                feature in classifier_data['growacq'][benchmark]['obj_proba'] and 
                classifier in classifier_data['growacq'][benchmark]['obj_proba'][feature]):
                val = classifier_data['growacq'][benchmark]['obj_proba'][feature][classifier]['tot_q']
                max_val = max(max_val, val)
        ...
        for i, (classifier, classifier_name) in enumerate(classifiers.items()):
            ...
            if ('obj_proba' in classifier_data['growacq'][benchmark] and 
                feature in classifier_data['growacq'][benchmark]['obj_proba'] and 
                classifier in classifier_data['growacq'][benchmark]['obj_proba'][feature]):
                proba_prev_val = classifier_data['growacq'][benchmark]['obj_proba'][feature][classifier]['tot_q']
                # Calculate and store percentage for later use
                if baseline_value > 0:
                    percentages[feature][classifier]['proba_prev'].append((proba_prev_val / baseline_value) * 100)
            
            bars = plt.bar(x + i * bar_width - 0.4 + bar_width/2, [proba_val, proba_prev_val], bar_width, 
        ...
        plt.xticks(x, ['Proba', 'Proba from [46]'], rotation=0)
        ...
        plt.tight_layout()
        plt.savefig(f'experiments/plots/q3_appendix/{benchmark}_{feature}_queries.png', bbox_inches='tight', dpi=300)
        plt.close()
        ...
        for classifier in classifiers:
            ...
            if ('obj_proba' in classifier_data['growacq'][benchmark] and 
                feature in classifier_data['growacq'][benchmark]['obj_proba'] and 
                classifier in classifier_data['growacq'][benchmark]['obj_proba'][feature]):
                val = (classifier_data['growacq'][benchmark]['obj_proba'][feature][classifier]['tot_q'] / baseline_value) * 100
                max_val = max(max_val, val)
        ...
        for i, (classifier, classifier_name) in enumerate(classifiers.items()):
            ...
            proba_prev_percentage = (proba_prev_val / baseline_value) * 100 if baseline_value > 0 else 0
            
            bars = plt.bar(x + i * bar_width - 0.4 + bar_width/2, [proba_percentage, proba_prev_percentage], bar_width, 
                    label=classifier_name, color=classifier_colors[classifier], edgecolor='black', linewidth=1.5)
        ...
        plt.tight_layout()
        plt.savefig(f'experiments/plots/q3_appendix/{benchmark}_{feature}_queries_percentage.png', bbox_inches='tight', dpi=300)
        plt.close()
...
for feature in ['simple_rel', 'rel_dim', 'rel_dim_block']:
    ...
    for classifier in classifiers:
        ...
        avg_proba_prev = np.mean(percentages[feature][classifier]['proba_prev']) if percentages[feature][classifier]['proba_prev'] else 0
        max_val = max(max_val, avg_proba, avg_proba_prev)
    ...
    for i, (classifier, classifier_name) in enumerate(classifiers.items()):
        ...
        bars = plt.bar(x + i * bar_width - 0.4 + bar_width/2, [avg_proba, avg_proba_prev], bar_width, 
                label=classifier_name, color=classifier_colors[classifier], edgecolor='black', linewidth=1.5)
    ...
    plt.tight_layout()
    plt.savefig(f'experiments/plots/q3_appendix/average_{feature}_queries.png', bbox_inches='tight', dpi=300)
    plt.close() 