...
def parse_averaged_findc_results(csv_file: str) -> Dict[str, Any]:
    """
    Parse the averaged results CSV file into a nested dictionary structure.
    Structure: {algorithm: {benchmark: {objective: {features: {findc: metrics}}}}}
    ...
    metric_columns = [col for col in df.columns if col not in 
                     ['Algorithm', 'Benchmark', 'Objective', 'Features', 'Findc', 'Rows Used']]
    ...
    for algorithm in algorithms:
        ...
        for benchmark in benchmarks:
            ...
            for _, row in valid_combinations.iterrows():
                ...
                # Get unique findcs for this combination
                findcs = combination_df['Findc'].unique()
                
                for findc in findcs:
                    findc_df = combination_df[combination_df['Findc'] == findc]
                    
                    if not findc_df.empty:
                        # Get the metrics for this combination
                        metrics = findc_df[metric_columns].iloc[0].to_dict()
                        results[algorithm][benchmark][objective][feature][findc] = metrics
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Parse averaged findc results into a nested dictionary structure')
    parser.add_argument('--input', type=str, default='experiments/aggregated_results/findc_results_averaged_results.csv',
                      help='Path to the averaged results CSV file (default: experiments/aggregated_results/findc_results_averaged_results.csv)')
    parser.add_argument('--output', type=str, default='experiments/parsed_results/parsed_findc_results.json',
                      help='Path to save the JSON output (default: experiments/parsed_results/parsed_findc_results.json)')
    ...
    # Parse the results
    results = parse_averaged_findc_results(args.input)
    ...
    for algorithm, benchmarks in results.items():
        ...
        for benchmark, objectives in benchmarks.items():
            ...
            for objective, features in objectives.items():
                ...
                for feature, findcs in features.items():
                    print(f"      Features: {feature}")
                    print(f"      Number of findcs: {len(findcs)}")
