...
def parse_averaged_findscope_results(csv_file: str) -> Dict[str, Any]:
    """
    Parse the averaged results CSV file into a nested dictionary structure.
    Structure: {algorithm: {benchmark: {objective: {features: {findscope: metrics}}}}}
    ...
    metric_columns = [col for col in df.columns if col not in 
                     ['Algorithm', 'Benchmark', 'Objective', 'Features', 'Findscope', 'Rows Used']]
    ...
    for algorithm in algorithms:
        ...
        for benchmark in benchmarks:
            ...
            for _, row in valid_combinations.iterrows():
                ...
                # Get unique findscopes for this combination
                findscopes = combination_df['Findscope'].unique()
                
                for findscope in findscopes:
                    findscope_df = combination_df[combination_df['Findscope'] == findscope]
                    
                    if not findscope_df.empty:
                        # Get the metrics for this combination
                        metrics = findscope_df[metric_columns].iloc[0].to_dict()
                        results[algorithm][benchmark][objective][feature][findscope] = metrics
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Parse averaged findscope results into a nested dictionary structure')
    parser.add_argument('--input', type=str, default='experiments/aggregated_results/findscope_results_averaged_results.csv',
                      help='Path to the averaged results CSV file (default: experiments/aggregated_results/findscope_results_averaged_results.csv)')
    parser.add_argument('--output', type=str, default='experiments/parsed_results/parsed_findscope_results.json',
                      help='Path to save the JSON output (default: experiments/parsed_results/parsed_findscope_results.json)')
    ...
    # Parse the results
    results = parse_averaged_findscope_results(args.input)
    ...
    for algorithm, benchmarks in results.items():
        ...
        for benchmark, objectives in benchmarks.items():
            ...
            for objective, features in objectives.items():
                ...
                for feature, findscopes in features.items():
                    print(f"      Features: {feature}")
                    print(f"      Number of findscopes: {len(findscopes)}")
