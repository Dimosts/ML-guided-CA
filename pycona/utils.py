...
class Objectives:
    ...
    def qgen_objectives(cls):
        ...
        """
        from .query_generation.qgen_obj import obj_max_viol, obj_min_viol, obj_proba, obj_proba2, obj_proba3, obj_class
        return [obj_max_viol, obj_min_viol, obj_proba, obj_proba2, obj_proba3, obj_class]
    ...
    def findscope_objectives(cls):
        ...
        """
        from .find_scope.findscope_obj import split_half, split_proba, cb_split_half_vars, cb_split_proba
        return [split_half, split_proba, cb_split_half_vars, cb_split_proba]
...
def get_relation_no_constants(c, gamma):
    """
    Get the relation index of a constraint in a given language.
    ...
    for i in range(len(gamma)):
        ...
        constants_dict = {}
        for cnst in get_constant(c):
            constants_dict[cnst] = 1
        constraint = replace_constants(c, constants_dict)
...
def replace_constants(constraint, const_mapping):
    """
    Replace the constants in a constraint using a dictionary mapping previous constants to new ones.

    :param constraint: The constraint to replace the constants in.
    :param const_mapping: Dictionary mapping previous constants to new ones.
    :return: The new expression with the constants replaced.
    """
    if isinstance(constraint, _NumVarImpl):
        return constraint
    elif isinstance(constraint, Expression):
        ...
        for argument in constraint.args:
            if isinstance(argument, _NumVarImpl):
                ...
                new_args.append(const_mapping.get(argument, argument))
            else:
                new_args.append(replace_constants(argument, const_mapping))
        # Replace the arguments in the copied expression
        new_constraint.update_args(new_args)
        return new_constraint
    elif isinstance(constraint, (int, float)):
        return const_mapping.get(constraint, constraint)
    else:
        return constraint
