...
from pycona.predictor.utils import get_divisors, average_difference
...
class FeaturesRelDimBlock(FeaturesRelDim):
    ...
    def featurize_constraint(self, c):
        ...
        for j in range(self._max_ndims):
            ...
            max_divisors = max(len(self._dim_divisors[var_name][j]) for var_name in range(len(self.var_names)))
            var_names = [self.var_names.index(get_var_name(scope[var])) for var in range(len(scope))]
            if max_divisors > 0:
                # Calculate block indices for each variable based on its dimension and divisors
                vars_block = []
                for divisor in range(self._max_divisors[j]):
                    block = []
                    for var in range(len(scope)):
                        var_idx = dim[j][var]  # Get the variable's index
                        var_name_idx = var_names[var]  # Get the variable's name index
                        divisor_val = self._dim_divisors[var_name_idx][j][divisor]  # Get the divisor value
                        block.append(var_idx // divisor_val)  # Calculate which block this variable belongs to
                    vars_block.append(block)

                for l in range(self._max_divisors[j]):
                    block_same = all([vars_block[l][var] == vars_block[l][0] for var in range(len(vars_block[l]))])
                    features.append(block_same)
            else:
                for _ in range(self._max_divisors[j]):
                    features.append(True)
        ...
        for j in range(self._max_ndims):
            ...
            if max_divisors > 0:
                # Calculate block indices for each variable based on its dimension and divisors
                vars_block = []
                for divisor in range(max_divisors):
                    block = []
                    for var in range(len(scope)):
                        var_idx = dim[j][var]  # Get the variable's index
                        var_name_idx = var_names[var]  # Get the variable's name index
        ...
        if len(features) != len(self._features):
            raise ValueError(f"Number of features: {len(features)} does not match the number of features in the dictionary: {len(self._features)}")
        return features
