...
import numpy as np
import argparse
from experiments.utils.experiment_utils import BENCHMARKS, ALGORITHMS, OBJECTIVES, FEATURES, FINDSCOPE

def parse_filename(filename):
    """Parse filename in format benchmark_algorithm_objective_features_findscope.csv
    ...
    # The remaining part is the findscope
    findscope = remaining
            
    return benchmark, algorithm, objective, features, findscope


def process_findscope_results(results_folder):
    ...
    for benchmark_dir in benchmark_dirs:
        ...
        for file in csv_files:
            try:
                ...
                benchmark, algorithm, objective, features, findscope = parse_filename(filename)
                
                if None in (benchmark, algorithm, objective, features, findscope):
                ...
                findscope_info.append(findscope)
    ...
    for i, df in enumerate([means_df, stderr_df]):
        ...
        df['Findscope'] = findscope_info
        df['Rows Used'] = rows_used_info
    
        # Reorder columns to put metadata first
        cols = ['Algorithm', 'Benchmark', 'Objective', 'Features', 'Findscope', 'Rows Used'] + \
               [col for col in df.columns if col not in ['Algorithm', 'Benchmark', 'Objective', 'Features', 'Findscope', 'Rows Used']]
        if i == 0:
            means_df = df[cols]
        else:
            stderr_df = df[cols]
...
if __name__ == "__main__":
    ...
    process_findscope_results(args.folder) 
